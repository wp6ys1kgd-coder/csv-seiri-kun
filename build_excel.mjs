...
result.getRange('A3:D200').formulasR1C1=[["=IF(入力!RC=\"\",\"\",入力!RC)","=IF(入力!RC=\"\",\"\",入力!RC)","=IF(入力!RC=\"\",\"\",入力!RC)","=IF(入力!RC=\"\",\"\",入力!RC)"]];
guide.getRange('A1:A6').values=[['CSV整理くん Excel版'],['1. 入力シートの見出しを残し、A3:D200へCSVデータを貼り付けます。'],['2. 整理結果シートで空白行を除いた結果を確認します。'],['3. 必要な範囲をCSVとして保存します。'],['この版はマクロを使わないため、警告なしで開けます。'],['複雑な重複削除や自動並べ替えは次版で追加予定です。']];
