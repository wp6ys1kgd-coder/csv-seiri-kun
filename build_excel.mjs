...
result.getRange('A3').formulas=[["=IF(設定!B2=\"氏名\",_xlfn.SORT(_xlfn.UNIQUE(_xlfn.FILTER(入力!A3:D200,入力!A3:A200<>\"\",\"\")),1,1),IF(設定!B2=\"メールアドレス\",_xlfn.SORT(_xlfn.UNIQUE(_xlfn.FILTER(入力!A3:D200,入力!A3:A200<>\"\",\"\")),2,1),IF(設定!B2=\"購入金額\",_xlfn.SORT(_xlfn.UNIQUE(_xlfn.FILTER(入力!A3:D200,入力!A3:A200<>\"\",\"\")),4,1),_xlfn.SORT(_xlfn.UNIQUE(_xlfn.FILTER(入力!A3:D200,入力!A3:A200<>\"\",\"\")),3,1))))"]];
...
settings.getRange('A1:C3').values=[['設定','',''],['並べ替え列','登録日',''],['選択肢','氏名 / メールアドレス / 登録日 / 購入金額','']];
